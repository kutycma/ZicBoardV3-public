...
class OrderController extends Controller
{
    ...
    public function detail(Request $request)
    {
        ...
        $order['try_out_plan_id'] = (int)config('zicboard.try_out_plan_id');
        if (!$order['plan']) {
    ...
    public function save(OrderSave $request)
    {
        ...
        $requestedSubscriptionId = $request->input('subscription_id');
        $subscription = $requestedSubscriptionId
            ? $subscriptionService->getForUser($user, $requestedSubscriptionId)
            : null;
        ...
        if ($requestedSubscriptionId && !$subscription) {
        ...
        if ($isWebcon && !$isWebconPlan) {
            abort(500, __('Subscription plan does not exist'));
        }

        if ((!$subscription || (int)$subscription->plan_id !== (int)$plan->id) && !$planService->haveCapacity() && $request->input('period') !== 'reset_price') {
        ...
        if ($request->input('period') === 'reset_price') {
            if (!$subscription || !$subscriptionService->isAvailable($subscription) || $plan->id !== $subscription->plan_id) {
                abort(500, __('Subscription has expired or no active subscription, unable to purchase Data Reset Package'));
            }
        }

        if (!$isWebconPlan && ((!$plan->show && !$plan->renew) || (!$plan->show && (!$subscription || $subscription->plan_id !== $plan->id)))) {
            if ($request->input('period') !== 'reset_price') {
                abort(500, __('This subscription has been sold out, please choose another subscription'));
            }
        }

        if (!$plan->renew && $subscription && $subscription->plan_id == $plan->id && $request->input('period') !== 'reset_price') {
            abort(500, __('This subscription cannot be renewed, please change to another subscription'));
        }


        if (!$isWebconPlan && !$plan->show && $plan->renew && (!$subscription || !$subscriptionService->isAvailable($subscription))) {
