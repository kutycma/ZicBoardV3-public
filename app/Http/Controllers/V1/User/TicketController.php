...
class TicketController extends Controller
{
    ...
    public function save(TicketSave $request)
    {
        try {
            ...
            $ticketStatus = config('zicboard.ticket_status', 0);

            switch ($ticketStatus) {
                case 0:
                    // Mở hoàn toàn, không chặn ticket nào
                    break;
                case 1:
                    // Chỉ người dùng có đơn hàng đã thanh toán
                ...
                case 2:
                    // Chặn toàn bộ ticket
                    throw new \Exception(__('Gói hiện tại không cho phép tạo ticket'));
                    break;
                default:
                    // Xử lý trạng thái không xác định
                    throw new \Exception(__('Trạng thái ticket không xác định'));
            }
    ...
    {
        if ((int)config('zicboard.withdraw_close_enable', 0)) {
        ...
        if (
			!in_array(
				...
				config(
					'zicboard.commission_withdraw_method',
    ...
    private function sendNotify(Ticket $ticket, string $message, $userid = null)
	{
		...
		if (!empty($userid)) {
			...
			if ($user) {
				$transfer_enable = $this->getFlowData($user->transfer_enable); // Tổng dung lượng
				$remaining_traffic = $this->getFlowData($user->transfer_enable - $user->u - $user->d); // Dung lượng còn lại
				$u = $this->getFlowData($user->u); // Tải lên
				$d = $this->getFlowData($user->d); // Tải xuống
				$expired_at = date("Y-m-d H:i:s", $user->expired_at); // Thời gian hết hạn
				$ip_address = request()->ip();
				$location =  "Chưa tra cứu vị trí bên ngoài";

				$plan = Plan::where('id', $user->plan_id)->first();
				$planName = $plan ? $plan->name : 'Không tìm thấy thông tin gói'; // Check if plan data is available

				$money = $user->balance / 100;
				$affmoney = $user->commission_balance / 100;
				$telegramService->sendMessageWithAdmin("📮Nhắc nhở ticket #{$ticket->id}\n———————————————\nEmail:\n`{$user->email}`\nVị trí người dùng:\n`{$location}`\nIP:\n{$ip_address}\nGói và dung lượng:\n`{$planName} of {$transfer_enable}/{$remaining_traffic}`\nTải lên/Tải xuống:\n`{$u}/{$d}`\nThời gian hết hạn:\n`{$expired_at}`\nSố dư / hoa hồng:\n`{$money}/{$affmoney}`\nChủ đề: \n`{$ticket->subject}`\nNội dung:\n {$message} ", true);
			} else {
    ...
    {
        $g = $b / (1024 * 1024 * 1024); // Chuyển đổi dữ liệu dung lượng
        $m = $b / (1024 * 1024);
        if ($g >= 1) {
            $text = round($g, 2) . "GB";
        } else {
