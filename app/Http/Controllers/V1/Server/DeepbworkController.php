...
class DeepbworkController extends Controller
{
    ...
    public function user(Request $request)
    {
        ...
        foreach ($users as $user) {
            $user->v2ray_user = [
                "uuid" => $user->uuid,
                "email" => sprintf("%s@zicboard.user", $user->uuid),
                "alter_id" => 0,
                "level" => 0,
            ];
            unset($user['uuid']);
...
//         Log::info('serverSubmitData:' . $request->input('node_id') . ':' . request()->getContent() ?: json_encode($_POST));
        ...
        if (!$server) {
            return response([
                'ret' => 0,
                'msg' => 'Máy chủ không tồn tại'
            ]);
        }
        $data = request()->getContent() ?: json_encode($_POST);
        $data = json_decode($data, true);
        $online = is_array($data) ? count($data) : 0;
        Cache::put(CacheKey::get('SERVER_VMESS_ONLINE_USER', $server->id), $online, 3600);
        ServerLoadIpOnline::record($request, $server, 'vmess', $online);
    ...
    private function getV2RayConfig(int $nodeId, int $localPort)
    {
        ...
        $json->log->loglevel = (int)config('zicboard.server_log_enable') ? 'debug' : 'none';
    ...
    {
        $domainRules = array_filter(explode(PHP_EOL, config('zicboard.server_v2ray_domain')));
        $protocolRules = array_filter(explode(PHP_EOL, config('zicboard.server_v2ray_protocol')));
