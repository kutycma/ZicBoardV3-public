...
class PlanController extends Controller
{
    ...
    public function drop(Request $request)
    {
        if (Order::where('plan_id', $request->input('id'))->first()) {
            abort(500, 'Gói này có đơn hàng nên không thể xóa');
        }
        if (User::where('plan_id', $request->input('id'))->first()) {
            abort(500, 'Gói này có người dùng nên không thể xóa');
        }
        if (UserSubscription::where('plan_id', $request->input('id'))->first()) {
            abort(500, 'Gói này đang có đăng ký nên không thể xóa');
        }
        if ($request->input('id')) {
            $plan = Plan::find($request->input('id'));
            if (!$plan) {
                abort(500, 'ID gói đăng ký này không tồn tại');
            }
        }
        return response([
            'data' => $plan->delete()
