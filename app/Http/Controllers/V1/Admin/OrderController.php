...
class OrderController extends Controller
{
    ...
    public function paid(Request $request)
    {
        ...
        if ($order->status !== 0) abort(500, 'Chỉ có thể thao tác với đơn hàng đang chờ thanh toán');
    ...
    public function assign(OrderAssign $request)
    {
        ...
        if (!$plan) {
            abort(500, 'Gói đăng ký này không tồn tại');
        }

        $userService = new UserService();
        if ($userService->isNotCompleteOrderByUserId($user->id)) {
            abort(500, 'Người dùng này còn đơn hàng chờ thanh toán, không thể phân bổ');
        }
        ...
        $subscription = $request->input('subscription_id')
            ...
            : $subscriptionService->getRenewableForPlan($user, (int)$plan->id);
