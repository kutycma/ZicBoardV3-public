...
class AdminRoute
{
    public function map(Registrar $router)
    {
        $router->group([
            ...
            foreach (['server/zicnode', 'server/v2node'] as $prefix) {
                $router->group([
                    'prefix' => $prefix
                ], function ($router) {
                    $router->post('save', 'V1\\Admin\\Server\\ZicnodeController@save');
                    $router->post('drop', 'V1\\Admin\\Server\\ZicnodeController@drop');
                    $router->post('update', 'V1\\Admin\\Server\\ZicnodeController@update');
                    $router->post('copy', 'V1\\Admin\\Server\\ZicnodeController@copy');
                });
            }
            // Order
            ...
            $router->post('/device/ban', 'V1\\Admin\\UserDeviceController@ban');
            $router->post('/device/resetUser', 'V1\\Admin\\UserDeviceController@resetUser');
            // Stat
            ...
            $router->post('/ticket/confirmCommission', 'V1\\Admin\\TicketController@confirmCommission');
            // Coupon
