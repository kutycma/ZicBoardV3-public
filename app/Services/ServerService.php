...
class ServerService
{
    public function getAvailableVless(User $user): array
    {
        ...
        foreach ($server as $key => $v) {
            ...
            $servers[] = ProtectedFeatureService::redactServerSecrets($server[$key]->toArray());
        }
    ...
    public function getAvailableZicnode(User $user)
    {
        $servers = [];
        $model = ServerZicnode::orderBy('sort', 'ASC');
        $zicnode = $model->get()->keyBy('id');
        foreach ($zicnode as $key => $v) {
            if (!$v['show']) continue;
            $zicnode[$key]['type'] = 'zicnode';
            $zicnode[$key]['last_check_at'] = Cache::get(CacheKey::get('SERVER_ZICNODE_LAST_CHECK_AT', $v['id']));
            if (!in_array($user->group_id, $v['group_id'])) continue;
            if (isset($zicnode[$v['parent_id']])) {
                $zicnode[$key]['last_check_at'] = Cache::get(CacheKey::get('SERVER_ZICNODE_LAST_CHECK_AT', $v['parent_id']));
                $zicnode[$key]['created_at'] = $zicnode[$v['parent_id']]['created_at'];
            }
            $servers[] = ProtectedFeatureService::redactServerSecrets($zicnode[$key]->toArray());
        }
        return $servers;
    }
    ...
    public function getAvailableUsers($groupId)
    {
        ...
        foreach ($users as $user) {
            ...
            foreach ($slotsByUser->get($user->id, collect()) as $slot) {
                ...
                $slotUser->uuid = $slot->uuid;
                $slotUser->device_limit = null;
                $nodeUsers->push($slotUser);
            }
        }
    ...
    public function getAllZicnode()
    {
        $servers = ServerZicnode::orderBy('sort', 'ASC')
            ->get()
            ->toArray();
        foreach ($servers as $k => $v) {
            $servers[$k]['type'] = 'zicnode';
            if (isset($v['padding_scheme'])) {
                $servers[$k]['padding_scheme'] = json_encode($v['padding_scheme']);
            }

            $serverApiUrl = trim((string) config('zicboard.server_api_url', ''));
            $apiHost = rtrim($serverApiUrl !== '' ? $serverApiUrl : (string) config('zicboard.app_url'), '/');
            ...
            $installScriptUrlArg = escapeshellarg((string) $installScriptUrl);
            $servers[$k]['install_command'] = sprintf(
                'wget -O zicnode-install.sh %s && bash zicnode-install.sh --api-host %s --node-id %d --api-key %s',
                $installScriptUrlArg,
                $apiHostArg,
                $nodeId,
                $apiKeyArg
    ...
    public function getServer($serverId, $serverType)
    {
        switch ($serverType) {
            case 'zicnode':
            case 'v2node':
                return ServerZicnode::find($serverId);
            case 'vmess':
